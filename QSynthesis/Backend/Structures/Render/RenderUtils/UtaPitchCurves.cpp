...
double UtaPitchCurves::f_type(const Qs::PitchPoint &ptype, const double &x1, const double &y1,
...
QVector<int> UtaPitchCurves::convert_from_vector_point(
    ...
    const QVector<QControlPoint> &nextNote, const QVector<double> &nextVBR, const double &nextPre,
    const double &nextOve, const int &nextLength, const QVector<QControlPoint> &prevNote,
    ...
    QVector<QControlPoint>::size_type i, j, k; // Self, successor, precursor counter
...
int UtaPitchCurves::find_impact(const QVector<QControlPoint> &portamento,
    ...
    int basePitch;
    QVector<QControlPoint>::size_type &i = startIndex; // Self counter
    double x1, y1, x2, y2;
    Qs::PitchPoint ptype;
    bool tooLeft;
