...
#include "QUtils.h"
#include "RenderUtils/UtaPitchCurves.h"
...
void ResamplerArgs::setToneName(int noteNum) {
    m_toneName = tone_number_to_tone_name(noteNum);
}
...
void ResamplerArgs::setPitchCurves(QVector<int> pitches) {
...
QStringList ResamplerArgs::paramsArguments() const {
    ...
    list << num_to_qstring(m_intensity);
    list << num_to_qstring(m_modulation);

    if (m_toBase64) {
        list << "!" + num_to_qstring(m_tempo);
        list << UtaPitchCurves::encode_from_vector(m_pitchCurves);
    } else {
        // No using Base 64
        if (m_pitchCurves.isEmpty()) {
            list << num_to_qstring(0) + "Q" + num_to_qstring(m_tempo);
        } else {
            list << num_to_qstring(m_pitchCurves.front()) + "Q" + num_to_qstring(m_tempo);

            for (int i = 1; i < m_pitchCurves.size(); ++i) {
                list << num_to_qstring(m_pitchCurves.at(i));
            }
        }
    }
...
QStringList ResamplerArgs::fullArguments() const {
    ...
    list << num_to_qstring(m_velocity); // Arg 4: Consonant Velocity
    list << m_flags;                    // Arg 5: Flags

    list << num_to_qstring(m_genonSettings.mOffset);   // Arg 6: Offset (Oto)
    list << num_to_qstring(m_realLength);              // Arg 7: Corrected Duration
    list << num_to_qstring(m_genonSettings.mConstant); // Arg 8: Consonant (Oto)
    list << num_to_qstring(m_genonSettings.mBlank);    // Arg 9: Blank (Oto)
...
QVector<int> ResamplerArgs::pitchCurves() const {
...
void ResamplerArgs::setGenonSettings(const QGenonSettings &genonSettings, bool useFileName) {
